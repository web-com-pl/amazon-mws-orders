            ...
            if (is_array($fieldType)) {
                if ($this->isComplexType($fieldType[0])) {
                    ...
                    if ($elements->length >= 1) {
                        foreach ($elements as $element) {
                            ...
                            $this->fields[$fieldName]['FieldValue'][] = new $class($element);
                        }
                    }
                } else {
                    $elements = $xpath->query("./a:$fieldName", $dom);
                ...
                if ($this->isComplexType($fieldType)) {
                    ...
                    if ($elements->length == 1) {
                        ...
                        $this->fields[$fieldName]['FieldValue'] = new $class($elements->item(0));
                    }
                } else {
        ...
        foreach ($this->fields as $fieldName => $field) {
            ...
            if (is_array($fieldType)) {
                ...
                if ($this->isComplexType($fieldType)) {
                    if (array_key_exists($fieldName, $array)) {
                        $class = '\\' . __NAMESPACE__ . '\\' . $fieldType;
                        $this->fields[$fieldName]['FieldValue'] = new $class($array[$fieldName]);
                    }
                } else {
                    if (array_key_exists($fieldName, $array)) {
